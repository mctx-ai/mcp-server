...
describe('createProgress() - edge cases', () => {
  ...
  it('notification object can be yielded in generator', () => {
    ...
    const gen = generator();
    expect(gen.next().value).toEqual({ type: 'progress', progress: 1, total: 3 });
    expect(gen.next().value).toEqual({ type: 'progress', progress: 2, total: 3 });
    expect(gen.next().value).toEqual({ type: 'progress', progress: 3, total: 3 });
