...
describe('user.attach()', () => {
  ...
  it('throws if mimeType is missing', () => {
    expect(() => {
      conversation(({ user }) => [
        user.attach('base64data'),
      ]);
    }).toThrow(/requires mimeType as second argument/);

    expect(() => {
      conversation(({ user }) => [
        user.attach('base64data', null),
      ]);
    }).toThrow(/requires mimeType as second argument/);
  });
